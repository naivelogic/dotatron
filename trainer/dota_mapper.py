...
def DotaDevkitMapper(dataset_dict, is_train=True):
    ...
    # to try> T.RandomFlip(prob=0.6, horizontal=True, vertical=False),
    # T.RandomBrightness(0.9, 1.1),
    random_cropper = dotakit_random_crop_instances(dataset_dict, image_np)
    if random_cropper is None:
        tfm_gens = [T.ResizeShortestEdge(short_edge_length=(500 ,640, 672, 800), max_size=800, sample_style='choice')]#, T.RandomFlip()]
        #tfm_gens = [T.ResizeShortestEdge([1024, 1024], 1024)] #[T.RandomFlip()]
    else:
        tfm_gens = [random_cropper, T.ResizeShortestEdge(short_edge_length=(500 ,640, 672, 800), max_size=800, sample_style='choice'), T.RandomFlip()]
                    #T.RandomBrightness(0.9, 1.1),
                    #T.RandomFlip(prob=0.5)]#, T.RandomFlip()]
    ...
    if "annotations" in dataset_dict:
        annos = [
            dota_transform_instance(obj, transforms, image_shape)
            #dota_transform_instance(obj, transforms, image.shape[:2]) 
...
def dotakit_random_crop_instances(dataset_dict, image_np):
    ...
    image_scale = 1 #np.random.uniform(0.09, 2) #2# 1 #original_gsd/target_gsd
    target_size = dataset_dict['height']
    ...
    #return None #T.CropTransform(0, 0, dataset_dict['height'], dataset_dict['height'])
...
def dota_annotations_to_instances(annos, image_size):
    ...
    if np.array(instances.gt_classes).any() in BIGGER_OBJECTS_TO_EXTEND:
        ...
        inside_ind = instances.gt_boxes.inside_box(image_size, boundary_threshold=5) #5
...
def dota_transform_instance(annotation, transforms, image_size, *, keypoint_hflip_indices=None):
    """
